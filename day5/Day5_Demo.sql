...
-- Ranking Functions 
-- RN if you no need repeate 
Select *
From (select *,Row_number() over(order by salary desc) as RN
	  from Employee) as Newtable
where RN=1

-- Danse Rank if data is repeate
Select *
From (select *,Dense_Rank() over(order by salary desc) as DR
	  from Employee) as NewTable
where DR=1

 -- Ntile (3)  النقص في الجروب الاخير لا يتعدي واحد وعايز يقسم

Select *
From (select *,Row_number() over(partition by LastName order by salary desc) as RN
	  from Employee) as Newtable
where RN=1

select * 
from (select * , Dense_rank() over(partition by lastName order by salary desc) as DR
from Employee) as Newtable
where DR = 1
...
-- any transaction will apply to table after merge
-- if you want to compare to two table or table and subquery
